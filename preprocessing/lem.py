...
def atomize(word):
    ...
    pattern_parsed_list = parse(word, relations=True, lemmata=True).split()[0]
    for patternParsed in pattern_parsed_list:
        ...
        #         print("word_has_caps", word_has_caps)
        #         print(word, root, pos)
        if word_has_caps:
...
def tokenize(text):
    ...
    for s in ar:
        ...
        #         s = s.lower()
        #         print("s: ", s)
        word_atoms = atomize(s)
        #         print('  word_atoms', word_atoms)
        #         print('  caps_chars', caps_chars)
        #         print('  type(caps_chars)', type(caps_chars))
        parsed += caps_chars + word_atoms
    #         print('  parsed', parsed)
    return parsed
...
def untokenize(atoms):
    ...
    while i < len(atoms) - 1:
        ...
        atom_next = atoms[i + 1]
        output += atoms[i]
        if (not ('"' in atom or "'" in atom or "..." in atom or
